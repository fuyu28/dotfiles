...
function M.expand(snippet)
  ...
  -- See: https://github.com/ConfigUtil/ConfigUtil/issues/3199
  ...
  if not ok then
    ...
    ConfigUtil[ok and "warn" or "error"](
      ([[%s
```%s
%s
...
function M.setup(opts)
  ...
  require("cmp.utils.snippet").parse = function(input)
    ...
    return ConfigUtil.cmp.snippet_preview(input)
  end
  ...
  cmp.event:on("confirm_done", function(event)
    if vim.tbl_contains(opts.auto_brackets or {}, vim.bo.filetype) then
      ConfigUtil.cmp.auto_brackets(event.entry)
    end
  end)
  cmp.event:on("menu_opened", function(event)
    ConfigUtil.cmp.add_missing_snippet_docs(event.window)
  end)
end
